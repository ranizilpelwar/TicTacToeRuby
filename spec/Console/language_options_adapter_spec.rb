...
RSpec.describe "a language options adapter" do
  context "method called get_input_choices" do
    it "raises an ArgumentError when options is nil" do
      expect{ LanguageOptionsAdapter.new("TicTacToeRuby.Console/Languages/").get_input_choices(nil) }.to raise_error(ArgumentError)
    end

    it "returns the expected list of input choices" do
      expected_choices = ["1", "2"]
      expect(LanguageOptionsAdapter.new("TicTacToeRuby.Console/Languages/").get_input_choices).to eq(expected_choices)
    end
  end
